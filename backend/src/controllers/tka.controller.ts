...
import { saveTkaGradesSchema } from '../validators/grade.validator';
import fs from 'fs';
...
export const getTkaGrades = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const saveTkaGrades = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = (req as any).user.tenantId;
    
    const validation = saveTkaGradesSchema.safeParse(req.body);
    ...
    logActivity({ req, action: 'SAVE_TKA_GRADES', entity: 'TkaGrade', description: `Menyimpan ${grades.length} nilai TKA (TP: ${activeYear.year})` });
...
export const exportTkaGrades = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = (req as any).user.tenantId;
    const { subjectType } = req.query;
...
export const importTkaGrades = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = (req as any).user.tenantId;
    ...
    await prisma.$transaction(async (tx) => {
      for (const item of gradesToUpsert) {
        ...
        await (tx.tkaGrade.upsert as any)({
          where: {
            tenantId_studentId_subjectType_academicYearId: {
              tenantId,
              studentId: student.id,
