...
interface BatchHaflahInvitationDownloaderProps {
  className?: string;
}

export default function BatchHaflahInvitationDownloader({ className }: BatchHaflahInvitationDownloaderProps) {
  const { user } = useAuth();
  ...
  const [hal, setHal] = useState('Undangan Haflah Akhirussanah');
  ...
  useEffect(() => {
    ...
    const savedNomor = localStorage.getItem(`${tenantPrefix}haflah_inv_nomorSurat`);
    const savedHal = localStorage.getItem(`${tenantPrefix}haflah_inv_hal`);
    const savedTgl = localStorage.getItem(`${tenantPrefix}haflah_inv_tanggalAcara`);
    const savedWaktu = localStorage.getItem(`${tenantPrefix}haflah_inv_waktuAcara`);
    const savedTempat = localStorage.getItem(`${tenantPrefix}haflah_inv_tempatAcara`);
  ...
  useEffect(() => { if (user?.tenantId) localStorage.setItem(`${tenantPrefix}haflah_inv_nomorSurat`, nomorSurat); }, [nomorSurat, user?.tenantId, tenantPrefix]);
  useEffect(() => { if (user?.tenantId) localStorage.setItem(`${tenantPrefix}haflah_inv_hal`, hal); }, [hal, user?.tenantId, tenantPrefix]);
  useEffect(() => { if (user?.tenantId) localStorage.setItem(`${tenantPrefix}haflah_inv_tanggalAcara`, tanggalAcara); }, [tanggalAcara, user?.tenantId, tenantPrefix]);
  useEffect(() => { if (user?.tenantId) localStorage.setItem(`${tenantPrefix}haflah_inv_waktuAcara`, waktuAcara); }, [waktuAcara, user?.tenantId, tenantPrefix]);
  useEffect(() => { if (user?.tenantId) localStorage.setItem(`${tenantPrefix}haflah_inv_tempatAcara`, tempatAcara); }, [tempatAcara, user?.tenantId, tenantPrefix]);
  ...
  const handleDownload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fileName = `Undangan_Haflah_Akhirussanah_${schoolName.replace(/\s+/g, '_')}_${new Date().getFullYear()}.pdf`;
      pdf.save(fileName);
      
      setModalOpen(false);
    } catch (err: any) {
      console.error('Error generating batch PDF:', err);
      const msg = err.response?.data?.message || 'Terjadi kesalahan saat mengunduh PDF undangan Haflah.';
      alert(msg);
    } finally {
      setDownloading(false);
          ...
          <div className="relative w-full max-w-lg bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700/50 bg-slate-800/50">
              <h3 className="text-lg font-semibold text-slate-100 flex items-center gap-2">
                ...
                Cetak Undangan Haflah Akhirussanah
              </h3>
              <button
            ...
            <div className="p-6 overflow-y-auto max-h-[80vh]">
              <form id="haflah-invitation-form" onSubmit={handleDownload} className="space-y-4">
                <div>
            ...
            <div className="px-6 py-4 border-t border-slate-700/50 bg-slate-800/50 flex justify-end gap-3">
              ...
              >
                {downloading ? <Loader2 className="w-4 h-4 animate-spin" /> : <PartyPopper className="w-4 h-4" />}
                ...
                <div key={student.id} className="invitation-page">
                  <div className="page-inner">
                    ...
                    <div className="invitation-body">
                      ...
                      <p style={{ marginTop: '16px' }}>
                        Sehubungan dengan telah berakhirnya masa pendidikan dan kegiatan belajar mengajar bagi siswa/siswi kelas akhir Tahun Pelajaran {batchData.academicYear}, maka kami mengharap kehadiran Bapak/Ibu Wali Murid pada acara <strong>Haflah Akhirussanah dan Pelepasan Siswa</strong> yang insya Allah akan diselenggarakan pada:
                      </p>
