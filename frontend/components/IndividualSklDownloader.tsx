...
export default function IndividualSklDownloader({ studentId, className }: IndividualSklDownloaderProps) {
  ...
  const handleDownload = async () => {
    try {
      ...
      const fileName = `SKL_${data.student.name.replace(/\s+/g, '_')}_${new Date().getFullYear()}.pdf`;
      pdf.save(fileName);
                  ...
                  <table className="identity-table">
                    <tbody>
                      {([
                        ...
                        ['Nomor Induk Siswa (NIS)',   docData.student.nis],
